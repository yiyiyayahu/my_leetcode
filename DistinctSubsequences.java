...
就是其实至少是d[i][j-1]，然后再加上，现在t[i]和s[j]匹配，这个选择固定了，那么就看d[i-1][j-1]是多少，就加多少了
比如rab，rabb：
  rab里面找到了一个rab，这个时候index=3的b没用上。之后让index=3的b匹配rab里面的b，发现rab里面找到了一个ra，就是1，那么rabb里面就能找到1+1=2个rab
